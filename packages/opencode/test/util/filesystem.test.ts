...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "deepseek-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "deepseek-code.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "deepseek-code.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "deepseek-code.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["deepseek-code.json", "deepseek-code.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "deepseek-code.json"),
        path.join(tmp.path, "deepseek-code.jsonc"),
        path.join(project, "deepseek-code.json"),
        path.join(project, "deepseek-code.jsonc"),
      ])
    })
  })
