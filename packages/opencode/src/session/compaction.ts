...
import { DEEPSEEK_COMPACTION_CONFIG, isDeepSeekModel } from "./deepseek-context"

// DeepSeek models get larger thresholds to leverage 1M context window
function getCompactionThresholds(modelID?: string) {
  if (modelID && isDeepSeekModel(modelID)) {
    return DEEPSEEK_COMPACTION_CONFIG
  }
  return {
...
// Default exports (used when model context is not available)
