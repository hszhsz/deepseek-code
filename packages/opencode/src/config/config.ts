...
function globalConfigFile() {
  const candidates = ["deepseek-code.jsonc", "deepseek-code.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "deepseek-code.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "deepseek-code.jsonc")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("deepseek-code", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".deepseek-code") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["deepseek-code.json", "deepseek-code.jsonc"]) {
